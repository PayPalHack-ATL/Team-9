...
class Game extends Emitter {
  constructor(io, onRemove) {
    ...
    const timer = setInterval(() => {
      const now = new Date();
      if (this.auction) {
        if (this.started_at && (now - this.started_at) >= this.auction.time * 1000) {
          this.finish();
        }
        if (this.finished_at && (now - this.finished_at) >= this.auction.time * this.players.length * 1000) {
          clearInterval(timer);
          this.remove();
          onRemove();
        }
  ...
  findPlayer(author, filter = () => true) {
    return this.players.find(player => author.username === player.user.username && filter(player));
  }
    ...
    if (!this.started_at) {
      ...
      Auction.count()
        .then(count => {
          ...
          return Auction.findOne().skip(random);
        })
        .then(auction => {
          this.auction = auction.toJSON({ req: {} });
          this.update();
        })
        .catch(console.error);
    }
